...
def show_reports(data):
    ...
    with col2:
        ...
        for t in resolved_tasks:
            assignee = t.get("assignee") or "Atanmamış"
            if assignee:
    ...
    if resolved_tasks:
        assignees = list(set([t.get("assignee") or "Atanmamış" for t in resolved_tasks]))
        ...
        for status in status_types:
            ...
            for assignee in assignees:
                c = sum(1 for t in resolved_tasks if (t.get("assignee") or "Atanmamış") == assignee and t.get("status") == status)
                counts.append(c)
