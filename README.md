...
**DeDev AI Project Command Center**, solo yazılım geliştiricileri, ürün yöneticileri ve yapay zeka mühendisleri için özel olarak tasarlanmış, **premium dark-glassmorphism** arayüzüne sahip, modüler, tam kapsamlı, yüksek güvenlikli ve MIT Lisanslı bir kurumsal **ERP ve Proje Yönetim Komuta Merkezidir**. 
...
DeDev Command Center, bir projenin fikirden üretime (production) kadar olan tüm süreçlerini tek noktadan yönetebilmeniz için **18 adet** birbiriyle entegre premium modül sunar:
...
### 📅 4. İnteraktif Zaman Tüneli (Timeline & Gantt)
* **Atanan Kişi Görselleştirmesi:** Gantt şeması üzerinde her görevin yanında doğrudan kimin sorumlu olduğunu gösteren `👤 {Atanan Kişi}` etiket entegrasyonu.
* **Zengin Hover Detayları (Tooltips):** Zaman çizgisi üzerindeki çubukların üzerine gelindiğinde; Proje Adı, Kategori, Atanan Kişi, Görev Durumu, Öncelik Derecesi, İş Gücü Efor Skoru, Hedef Bitiş Tarihi ve Detaylı Açıklama gibi tüm parametreleri pürüzsüzce sunar.
* **Milestone (Kilometre Taşları):** Projelerin altındaki kritik teslim tarihlerini ve aşamaları zaman çizelgesi üzerinde dinamik olarak konumlandırır. Hata korumalı checklist durum entegrasyonu ile sıfır KeyError garantisi sunar.
...
### 🧾 14. Sekmeli Fatura & Gelir Entegrasyonu (Billing & Invoicing)
...
* **Fatura Arşivleme & Arama:** Kesilen tüm faturaları tarih, proje ve fatura numarasına göre anında listeleyebilen, pürüzsüz arama filtrelerine sahip veritabanı entegrasyonu.
* **PDF Faturalama Altyapısı:** Kesilen faturaların yazdırılabilir standartta şablonlanması.

### 📂 15. ERP Arşiv Odası & PDF Görüntüleyici (Archive Room)
...
* **Tarayıcı İçi PDF Görüntüleyici:** Streamlit entegrasyonlu pürüzsüz PDF okuyucu ile belgeleri bilgisayarınıza indirmeden doğrudan tarayıcı ekranından okuyup inceleme desteği.
* **MIT Lisans Beyanı:** Yüklenen ve arşivlenen tüm belgelerin lisanslama kuralları ve bütünlük doğrulamaları ile arşivlenmesi.

### 📊 16. DeDev Findeks Skoru & ERP Portföy Analizi (Findeks Score)
* **Findeks Kredi Notu Entegrasyonu:** Şirketinizin ve projelerinizin finansal performansını, bütçe uyumluluğunu ve zamanında teslimat oranlarını analiz ederek dinamik bir Findeks Kredi Skoru (1 - 1900 puan arası) hesaplayan gelişmiş dairesel gösterge (Gauge Chart).
* **Baskıya Hazır Resmi Raporlama (MIT Licensed PDF/HTML):** Portföy analizi, finansal risk derecelendirmesi ve Findeks skorunu içeren resmi kurumsal raporu tek tıkla MIT Lisanslı ve kurumsal antetli kağıt düzeninde **baskıya hazır HTML/PDF** formatında indirme butonu.

### 📅 17. İnteraktif Proje Takvimi (Calendar View)
...
### 🚀 18. Canlı Sunucu & Dağıtım Takibi (Deployments Panel)
* Projelerin canlıya (production/staging) çıkış geçmişlerini, son dağıtım tarihlerini, sunucu sağlayıcılarını (Vercel, Heroku, AWS, DigitalOcean) ve canlı URL adreslerini takip eden DevOps paneli.
...
2. **Projeler İçin Güvenli XOR & SHA-256 Şifreleme (Secret Vault):**
   * Gizli projelerin hassas verileri, SHA-256 ile türetilen simetrik bir anahtar kullanılarak XOR tabanlı şifrelemeyle veritabanına (`data.json`) yazılır. Parola girilmeden bellek düzeyinde dahi çözülemez.

---

## ⚖️ Lisans

Bu proje **MIT Lisansı** ile lisanslanmıştır. Kurumsal Findeks ve ERP portföy rapor çıktılarında lisans şablonları ve telif korumaları otomatik olarak resmi ibareler halinde eklenmektedir.
...
└── views/                      # Kullanıcı Arayüzü (Sayfa Görünümleri)
    ...
    ├── admin_view.py           # Yönetici kontrolleri ve simülasyon hesap kılavuzu
